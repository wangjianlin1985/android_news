...
public class NewsService {
	...
	public String AddNews(News news) {
		...
		params.put("comFrom", news.getComFrom());
		params.put("hitNum", news.getHitNum() + "");
	...
	public List<News> QueryNews(News queryConditionNews) throws Exception {
		...
		if(queryConditionNews != null) {
			...
			urlString += "&comFrom=" + URLEncoder.encode(queryConditionNews.getComFrom(), "UTF-8") + "";
			urlString += "&addTime=" + URLEncoder.encode(queryConditionNews.getAddTime(), "UTF-8") + "";
		}
		...
		try {
			...
			for (int i = 0; i < length; i++) {
				...
				news.setComFrom(object.getString("comFrom"));
				news.setHitNum(object.getInt("hitNum"));
				news.setAddTime(object.getString("addTime"));
				newsList.add(news);
			}
		} catch (Exception e) {
			e.printStackTrace();
