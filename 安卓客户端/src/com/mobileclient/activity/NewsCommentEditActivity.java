...
public class NewsCommentEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.newscomment_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑新闻评论信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_content.getText().toString().equals("")) {
						Toast.makeText(NewsCommentEditActivity.this, "评论内容输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_commentTime.getText().toString().equals("")) {
						Toast.makeText(NewsCommentEditActivity.this, "评论时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传新闻评论信息*/
					NewsCommentEditActivity.this.setTitle("正在更新新闻评论信息，稍等...");
					String result = newsCommentService.UpdateNewsComment(newsComment);
	...
	private void initViewData() {
		...
		for (int i = 0; i < newsList.size(); i++) {
			if (newsComment.getNewsObj() == newsList.get(i).getNewsId()) {
				this.spinner_newsObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < userInfoList.size(); i++) {
			if (newsComment.getUserObj().equals(userInfoList.get(i).getUser_name())) {
				this.spinner_userObj.setSelection(i);
				break;
			}
		}
		this.ET_content.setText(newsComment.getContent());
		this.ET_commentTime.setText(newsComment.getCommentTime());
	}
	@Override
