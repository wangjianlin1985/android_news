...
public class PhotoShareEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.photoshare_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑图片分享信息");
		...
		iv_sharePhoto.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(PhotoShareEditActivity.this,photoListActivity.class);
			...
			public void onClick(View v) {
				try {
					...
					if(ET_photoTitle.getText().toString().equals("")) {
						Toast.makeText(PhotoShareEditActivity.this, "图片标题输入不能为空!", Toast.LENGTH_LONG).show();
						ET_photoTitle.setFocusable(true);
						ET_photoTitle.requestFocus();
						return;	
					}
					photoShare.setPhotoTitle(ET_photoTitle.getText().toString());
					if (!photoShare.getSharePhoto().startsWith("upload/")) {
						//如果图片地址不为空，说明用户选择了图片，这时需要连接服务器上传图片
						PhotoShareEditActivity.this.setTitle("正在上传图片，稍等...");
						String sharePhoto = HttpUtil.uploadFile(photoShare.getSharePhoto());
						PhotoShareEditActivity.this.setTitle("图片上传完毕！");
						photoShare.setSharePhoto(sharePhoto);
					} 
					/*验证获取分享时间*/ 
					if(ET_shareTime.getText().toString().equals("")) {
						Toast.makeText(PhotoShareEditActivity.this, "分享时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传图片分享信息*/
					PhotoShareEditActivity.this.setTitle("正在更新图片分享信息，稍等...");
					String result = photoShareService.UpdatePhotoShare(photoShare);
	...
	private void initViewData() {
		...
		this.TV_sharePhotoId.setText(sharePhotoId+"");
		this.ET_photoTitle.setText(photoShare.getPhotoTitle());
		...
		for (int i = 0; i < userInfoList.size(); i++) {
			if (photoShare.getUserInfoObj().equals(userInfoList.get(i).getUser_name())) {
				this.spinner_userInfoObj.setSelection(i);
				break;
			}
		}
		this.ET_shareTime.setText(photoShare.getShareTime());
	}
	@Override
