...
public class NewsTagEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.newstag_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑新闻标记信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_newsState.getText().toString().equals("")) {
						Toast.makeText(NewsTagEditActivity.this, "新闻状态输入不能为空!", Toast.LENGTH_LONG).show();
					...
					if(ET_tagTime.getText().toString().equals("")) {
						Toast.makeText(NewsTagEditActivity.this, "标记时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传新闻标记信息*/
					NewsTagEditActivity.this.setTitle("正在更新新闻标记信息，稍等...");
					String result = newsTagService.UpdateNewsTag(newsTag);
	...
	private void initViewData() {
		...
		this.TV_tagId.setText(tagId+"");
		for (int i = 0; i < newsList.size(); i++) {
			if (newsTag.getNewsObj() == newsList.get(i).getNewsId()) {
				this.spinner_newsObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < userInfoList.size(); i++) {
			if (newsTag.getUserObj().equals(userInfoList.get(i).getUser_name())) {
				this.spinner_userObj.setSelection(i);
				break;
			}
		}
		this.ET_newsState.setText(newsTag.getNewsState() + "");
		this.ET_tagTime.setText(newsTag.getTagTime());
	}
	@Override
