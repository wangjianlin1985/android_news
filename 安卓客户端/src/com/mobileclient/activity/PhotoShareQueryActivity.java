...
public class PhotoShareQueryActivity extends Activity {
	// 声明查询按钮
	...
	/*查询过滤条件保存到这个对象中*/
	private PhotoShare queryConditionPhotoShare = new PhotoShare();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.photoshare_query);
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("设置图片分享查询条件");
		...
		spinner_userInfoObj.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,int arg2, long arg3) {
				if(arg2 != 0)
					queryConditionPhotoShare.setUserInfoObj(userInfoList.get(arg2-1).getUser_name()); 
				else
					queryConditionPhotoShare.setUserInfoObj("");
			}
			@Override
		...
		btnQuery.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				try {
					/*获取查询参数*/
					queryConditionPhotoShare.setShareTime(ET_shareTime.getText().toString());
					Intent intent = getIntent();
					//这里使用bundle绷带来传输数据
					Bundle bundle =new Bundle();
					//传输的内容仍然是键值对的形式
					bundle.putSerializable("queryConditionPhotoShare", queryConditionPhotoShare);
