...
public class NewsCollectionEditActivity extends Activity {
	// 声明确定添加按钮
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 设置当前Activity界面布局
		setContentView(R.layout.newscollection_edit); 
		ImageView search = (ImageView) this.findViewById(R.id.search);
		search.setVisibility(View.GONE);
		TextView title = (TextView) this.findViewById(R.id.title);
		title.setText("编辑新闻收藏信息");
			...
			public void onClick(View v) {
				try {
					...
					if(ET_collectTime.getText().toString().equals("")) {
						Toast.makeText(NewsCollectionEditActivity.this, "收藏时间输入不能为空!", Toast.LENGTH_LONG).show();
					...
					/*调用业务逻辑层上传新闻收藏信息*/
					NewsCollectionEditActivity.this.setTitle("正在更新新闻收藏信息，稍等...");
					String result = newsCollectionService.UpdateNewsCollection(newsCollection);
	...
	private void initViewData() {
		...
		for (int i = 0; i < newsList.size(); i++) {
			if (newsCollection.getNewsObj() == newsList.get(i).getNewsId()) {
				this.spinner_newsObj.setSelection(i);
				break;
			}
		}
		for (int i = 0; i < userInfoList.size(); i++) {
			if (newsCollection.getUserObj().equals(userInfoList.get(i).getUser_name())) {
				this.spinner_userObj.setSelection(i);
				break;
			}
		}
		this.ET_collectTime.setText(newsCollection.getCollectTime());
	}
	@Override
