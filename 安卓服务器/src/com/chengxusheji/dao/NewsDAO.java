...
public class NewsDAO {
    ...
    public ArrayList<News> QueryNewsInfo(NewsClass newsClassObj,String newsTitle,String comFrom,String addTime,int currentPage) { 
    	Session s = factory.getCurrentSession();
    	...
    	if(!comFrom.equals("")) hql = hql + " and news.comFrom like '%" + comFrom + "%'";
    	if(!addTime.equals("")) hql = hql + " and news.addTime like '%" + addTime + "%'";
    ...
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public ArrayList<News> QueryNewsInfo(NewsClass newsClassObj,String newsTitle,String comFrom,String addTime) { 
    	Session s = factory.getCurrentSession();
    	String hql = "From News news where 1=1";
    	if(null != newsClassObj && newsClassObj.getNewsClassId()!=0) hql += " and news.newsClassObj.newsClassId=" + newsClassObj.getNewsClassId();
    	if(!newsTitle.equals("")) hql = hql + " and news.newsTitle like '%" + newsTitle + "%'";
    ...
    /*计算总的页数和记录数*/
    @Transactional(propagation=Propagation.NOT_SUPPORTED)
    public void CalculateTotalPageAndRecordNumber(NewsClass newsClassObj,String newsTitle,String comFrom,String addTime) {
        Session s = factory.getCurrentSession();
        String hql = "From News news where 1=1";
        if(null != newsClassObj && newsClassObj.getNewsClassId()!=0) hql += " and news.newsClassObj.newsClassId=" + newsClassObj.getNewsClassId();
        if(!newsTitle.equals("")) hql = hql + " and news.newsTitle like '%" + newsTitle + "%'";
